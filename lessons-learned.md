...
- **Invariant**: every Graviblocks target ships an explicit `main.swift`
...
- **Invariant**: every `NSTextView` in Graviblocks is built by the canonical
...
- **Invariant**: Graviblocks ships `NSOpenPanel` / `NSSavePanel` first.
...
- **Invariant**: in Graviblocks the build check MUST invoke at least one
...
- **Invariant**: Graviblocks's feature checks talk to the app via
...
- **Invariant**: each agent invocation in Graviblocks is wrapped by Go
...
- **Invariant**: every code-agent invocation in Graviblocks is sandwiched
...
- **Invariant**: in Graviblocks, no quality review starts until the
...
- **Always commit on a working tree.** The old loop refused to
  ...
  Graviblocks's orchestrator `git status --porcelain` checks both
  pre- and post-agent.
