---
name: graviblocks-mvc-appkit
description: Graviblocks's architectural contract — every feature lives in an NSViewController that owns its model, its view, AND its HTTP test-API routes. Use this skill on every code-writing turn for the Graviblocks project.
---

# Graviblocks MVC for macOS / AppKit

This skill is **mandatory** for every code change to the Graviblocks
...
- Plain Swift structs / classes in `Graviblocks/<Feature>/<Name>State.swift`.
...
- An `NSView` subclass in `Graviblocks/<Feature>/<Name>View.swift`.
...
- An `NSViewController` subclass in
  `Graviblocks/<Feature>/<Name>Controller.swift`.
...
This is the single most important contract in Graviblocks. Every
...
1. A route registered outside its controller. (e.g. `POST /editor/type`
   declared in `Graviblocks/App/TestAPI/Routes.swift`.)
